...
namespace RitualLogParser.UI
{
    public static class LogFileParser
    {
        ...
        public static ItemModel ParseItemModel(string itemString)
        {
            ...
            int cost = -1, deferMs = 0, ilvl = -1, totalMs = 0, tribFindMs = 0, costCompMs = 0;
            if(!int.TryParse(Regex.Match(firstSection, @"Cost: (.*)").Groups[1].Value, out cost)) cost = -1;
            int.TryParse(Regex.Match(firstSection, @"Defer: (.*)").Groups[1].Value, out deferMs);
            ...
            try
            {
                item = new ItemModel
                {
                    ...
                    ItemLevel = ilvl,
                    DeferTime = TimeSpan.FromMilliseconds(deferMs),
            ...
            {
                item = new ItemModel { Name = "Error Parsing Logfile" };
            }
            return item;
        }
    }
